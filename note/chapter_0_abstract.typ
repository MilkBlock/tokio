...
= Schedule 
== LIFO & Local Queue & Global Queue 
#table(
  columns: 2,
  [任务选取顺序], [Comment],
  [LIFO_SLOT],[后进先出，当线程执行完一个任务时，会根据是否yield从LIFO_SLOT中取一个任务\ \@tokio/src/runtime/scheduler/multi_thread/worker.rs:1097],
  [Local Queue],[线程私有的队列，用于存放当前线程需要执行的任务],
  [Global Queue],[全局的队列，用于存放需要被其他线程执行的任务]
)
...
= Driver 
#image("./driver.png",width: 70%)
可以看到图中 driver 拥有一个 io handle 
这个io handle 封装了 mio 库的
